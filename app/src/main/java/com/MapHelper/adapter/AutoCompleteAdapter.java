...
import com.MapHelper.model.PlaceAutoComplete;
...
public class AutoCompleteAdapter extends ArrayAdapter<PlaceAutoComplete> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
       ...
       // holder.name.setText(st.nextToken()+" "+desc_detail);
        try{
            holder.location.setText(st.nextToken());

            holder.location.setText(st.nextToken()==null?"":st.nextToken()+" "+desc_detail==null?"":desc_detail);
        }catch (Exception e){
